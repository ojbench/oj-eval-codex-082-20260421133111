...
int main(){
    ...
    for(int i=0;i<q;++i){
        ...
        else if(cmd=="3"){ int k; cin>>k; auto a=mp.ask(k%mp.len); for(size_t j=0;j<a.size();++j){ if(j) cout<<' '; cout<<a[j]; } cout<<"\n"; }
        else if(cmd=="4"){ auto all=mp.forEach(); for(size_t j=0;j<all.size();++j){ if(j) cout<<' '; cout<<'('<<all[j].key<<','<<all[j].value<<')'; } cout<<"\n"; }
